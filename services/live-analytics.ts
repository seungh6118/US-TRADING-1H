...
export function buildLiveSectorPerformance(stocks: StockSnapshot[]): SectorPerformance[] {
  ...
  return Array.from(grouped.entries())
    .map(([sector, members]) => {
      const performance1dPct = weightedAverage(members.map((stock) => ({ value: stock.quote.change1dPct, weight: stock.fundamentals.marketCapBn })));
      ...
      return {
        sector,
        etf: topNames.join(", "),
        performance1dPct: Number(performance1dPct.toFixed(2)),
        ...
        relativeStrength,
        score: Number(scoreFromMomentum(performance1dPct, performance5dPct, performance20dPct, performance60dPct, excess20dPct).toFixed(1)),
        drivers: topNames
...
export function buildLiveThemeSnapshots(stocks: StockSnapshot[]): ThemeSnapshot[] {
  ...
  return Array.from(grouped.entries())
    .map(([name, members]) => {
      ...
      const score =
        newsMentions === 0
          ? clamp(priceMomentumScore * 0.85 + sentimentScore * 0.15)
          : clamp(priceMomentumScore * 0.55 + sentimentScore * 0.2 + mentionScore * 0.25);

      let summary = `${name} 테마는 현재 종목별 선별이 중요합니다.`;
      if (score >= 75) {
        summary = `${name} 테마는 뉴스와 가격 반응이 함께 강해 상위 감시 대상으로 볼 만합니다.`;
      } else if (score >= 60) {
        summary = `${name} 테마는 단기 가격 반응이 강해 우선 감시할 가치가 있습니다.`;
      } else if (score < 45) {
